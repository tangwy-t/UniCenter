...
//
// 本框架刻意不实现 Down/回滚，原因：
//   - 数据迁移不可逆：种子数据（如 admin 用户、系统菜单）上线后
...
// ## 出了问题怎么办
//
// 写一个新的正向迁移修复（如 vNNN_fix_xxx.go），永不回头修改已发布的
...
//     再在下个版本迁移里删除。
//
// 新迁移文件命名：migrations/vNNN_seed_xxx.go（初始化种子按领域拆分）
// 或后续增量修复可用 vNNN_描述.go，注册见 register.go。
package migration

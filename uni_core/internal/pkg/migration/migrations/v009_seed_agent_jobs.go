...
//
// backfill 的 `invoke_params` 是 `{"hours":24}`（**不含** resolution），这是刻意的：
//
//   - **缺省 resolution = 5m**，即「回退 cursor_5m 到 now−24h，再重放 5m」——
//     与引入 resolution 之前逐字相同的行为，所以种子不必为了新能力改值；
//   - 这条定时任务守的是「原始点还在热层（24h 内）时把 5m 桶补回来」，它的窗口口径
//     属于 **Redis 原始点保留期**，与库里的保留期是两件不同的事实；
//   - 1h 那一支（`{"hours":720,"resolution":"1h"}` → 只回退 cursor_1h，重算交给下一轮
//     rollup）是**运维按需触发**的能力：它每轮会让 rollup 重算最多 30 天的小时，
//     不该被排成一条每小时都跑的定时任务。需要时改这一行的 params（或临时手跑一次）。
//
// **已有库**的语义（本迁移按 invoke_target 去重，已存在的行一律跳过，见 seedAgentJobs）：
// 上面这些说明只对「本迁移第一次跑时新建的行」生效；升级前就存在的 `{"hours":24}` 行
// 原样保留，而它的行为与今天的缺省**完全一致**（5m 支未变），不需要任何数据迁移。
//
